...
export default function VideoCallScreen() {
    const callMer = useSelector((state) => state.Utils.call);
    ...
    let offer = null;
    offer = useSelector((state) => state.Utils.call.data);
    ...
    let unprocessedRemoteCandidates = useRef([]);
    ...
    function onOpenModal() {
        try {
            ...
            if (callMer.data.isCaller) {
                socket.subscribeToEvent("answerOfferVideoCall", (data) => {
                    onAnswerOfferVideoCall(data);
                });

                socket.subscribeToEvent("noOneInRoom", (data) => {
                    Toast("Other user is not online");
                    dispatch(setCallShow(false));
                });
            }

            socket.subscribeToEvent("callDenied", (data) => {
                Toast("Other user denied call");
                dispatch(setCallShow(false));
            });

            socket.subscribeToEvent("iceCandidate", (data) => {
    ...
    function handleRemoteCandidate(receivedIceCandidate) {
        try {
    ...
    const onAnswerOfferVideoCall = async (answer) => {
        try {
...
const CallAnswer = (props) => (
    ...
    >
        ...
        <Path d="M25 12c0 .5 1.2 1 2.6 1 1.6 0 3.9 1.3 6 3.4 2.1 2.1 3.4 4.4 3.4 6 0 1.4.5 2.6 1 2.6 1.7 0 1-3.2-1.9-6.1C28.2 16 25 15 25 17z" />
    </Svg>
);

const CallDeny = (props) => (
