import {
  ...
  export default function DetailStatusScreen({ route }) {
    ...
    const handleDelete = useCallback(async (idPost) => {
      try {
    ...
    const postComment = useCallback(async () => {
      if (comment === '' && mediaFile === null) { // Change condition to check for null
          ...
          <View style={{ flex: 1, marginVertical: 5, paddingVertical: 10 }}>
            {comments.map((comment, index) => (
              <ItemComment
                key={index}
                item={comment}
                pressDeleteComment={() => handleDeleteComment(comment._id)}
              />
            ))}
          </View>
